...
class slider():
    ...
    def update_val(self, mouse_x):
        ...
        self.val = max(0,0.0, min(1.0, relative_x / self.rect.width))

    def draw(self, screen):
        pygame.draw.rect(screen,  (50, 50, 65), self.rect, border_radius=4)

        fill_width = int(self.vel * self.rect.width)
        if fill_width > 0:
            fill_rect = pygame.Rect(self.x, self.y, fill_width, self.rect.height)  
            pygame.draw.rect(screen, (100, 200,255), self.rect, 2, border_radius= 4)
